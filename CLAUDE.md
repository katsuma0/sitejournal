...
- Secret search commands: debugsearch (diagnostics), statsearch (rating stats), dummydata (left-skewed sample ratings), dummyhundop (confirm button; overwrites EVERYTHING to 5/5 with wishlists, notes, all themes, egg revealed near Markham incl Grundy Hemlock/Jack Pine, Bon Echo Hardwood, Canisbay all, Silent Lake random cg, Awenda Bear; never overwrites real entries), rendered by renderConsole as monospace .dbg cards with variable_name keys. Never list these in the Versions changelog.
